...
import { ExpandableContainer, EXPANDABLE_CONTAINER } from './expandable-container.directive'
...
export class ExpandableContent {
  ...
  constructor(
    @Inject(EXPANDABLE_CONTAINER) private container: ExpandableContainer,
  ...
  closeAtInitialization(): void {
    this.player = this.animationBuilder
      .build([style({ height: '*' }), style({ height: 0, visibility: 'hidden' })])
